...
public class CustomRedisTokenStore implements TokenStore {
    ...
    public OAuth2Authentication readAuthentication(OAuth2AccessToken token) {
        ...
        //是否开启token续签
        boolean isRenew = securityProperties.getOauth2().getRenew().getEnable();
        if (isRenew && auth2Authentication != null) {
            ...
            if (checkRenewClientId(clientAuth.getClientId())) {
                ...
                if (validitySeconds > 0) {
                    ...
                    if (expiresRatio <= securityProperties.getOauth2().getRenew().getTimeRatio()) {
                        //更新AccessToken过期时间
    ...
    private boolean checkRenewClientId(String clientId) {
        boolean result = true;
        //白名单
        List<String> includeClientIds = securityProperties.getOauth2().getRenew().getIncludeClientIds();
        //黑名单
        List<String> exclusiveClientIds = securityProperties.getOauth2().getRenew().getExclusiveClientIds();
